 ...
 *
 */
object _04_PassportProcessing {

  val birthYear = "byr"

  def validateBirthYear(input: String): Boolean = {
    Try(input.toInt).filter(y => 1920 <= y && y <= 2002).isSuccess
  }

  val issueYear = "iyr"

  def validateIssueYear(input: String): Boolean = {
    Try(input.toInt).filter(y => 2010 <= y && y <= 2020).isSuccess
  }

  val expirationYear = "eyr"

  def validateExpirationYear(input: String): Boolean = {
    Try(input.toInt).filter(y => 2020 <= y && y <= 2030).isSuccess
  }

  val height = "hgt"

  def validateHeight(input: String): Boolean = {
    val pattern = "^(\\d+)(cm|in)$".r

    input match {
      case pattern(value, "cm") => 150 <= value.toInt && value.toInt <= 193
  ...
  def validateHairColor(input: String): Boolean = {
  ...
  def validateEyeColor(input: String): Boolean = {
    val pattern = "^(amb|blu|brn|gry|grn|hzl|oth)$".r
  ...
  def validatePassportId(input: String): Boolean = {
    val pattern = "^\\d{9}$".r
  ...
  def isValid_part_01(lines: List[String]): Boolean = {
    val given: Set[String] = lines
      .map(_.split(" ").map(_.split(":").head).toSet)
      .foldLeft(Set.empty[String])(_ ++ _)

    (total -- `given`).subsetOf(Set("cid"))
  }

  def isValid_part_02(lines: List[String]): Boolean = {
    val pairs: Map[String, String] = lines
      .map(_.split(" ").map { pair =>
        val Array(k, v) = pair.split(":")
        k -> v
      }.toMap)
      .foldLeft(Map.empty[String, String])(_ ++ _)

    (total -- pairs.keySet).subsetOf(Set("cid")) &&
    validateBirthYear(pairs(birthYear)) &&
    validateIssueYear(pairs(issueYear)) &&
    validateExpirationYear(pairs(expirationYear)) &&
    validateHeight(pairs(height)) &&
    validateHairColor(pairs(hairColor)) &&
    validateEyeColor(pairs(eyeColor)) &&
    validatePassportId(pairs(passportId))
  }
  @tailrec
  def passports(
      remaining: List[String],
      acc: List[List[String]] = Nil
  ): List[List[String]] = {
    if (remaining.isEmpty) acc
    else {
      val (passport, rest) =
        remaining.dropWhile(_.trim.isEmpty).span(_.trim.nonEmpty)

      passports(rest, if (passport.nonEmpty) passport :: acc else acc)
    }
  }
  ...
  def main(args: Array[String]): Unit = {
    ...
    val input =
      """eyr:2027
        ...
        |iyr:1930 eyr:2024
        |
        |ecl:oth hcl:#602927 eyr:2025 iyr:2013 hgt:151cm byr:1992 pid:812583062""".stripMargin
        .split("\n")
        .toList

    println {
      passports(input).count(isValid_part_02)
    }
