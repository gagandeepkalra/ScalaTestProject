 ...
 */
object _05_BinaryBoarding {

  @tailrec
  def findRow(code: List[Char], low: Int = 0, high: Int = 127): Int = {
    val m = (low + high) / 2
    code.headOption match {
      case Some('F') => findRow(code.tail, low, m)
      case Some('B') => findRow(code.tail, m + 1, high)
      case None      => low
    }
  }

  @tailrec
  def findCol(code: List[Char], low: Int = 0, high: Int = 7): Int = {
    val m = (low + high) / 2
    code.headOption match {
      case Some('L') => findCol(code.tail, low, m)
      case Some('R') => findCol(code.tail, m + 1, high)
      case None      => low
    }
  }

  def main(args: Array[String]): Unit = {
    val inputs = """BBFFFBBLLR
                   |BBFFFFFLRL
                   |BFBFFFBLRR
                   |FBBFFFFRLL
                   |FFBFFFBLRR
                   ...
                   |FFBBFFFRRR
                   |FBBBFFBRRL
                   |BFFFFFFRRR""".stripMargin.split("\n")

    val allIds = inputs.map(code =>
      findRow(code.slice(0, 7).toList) * 8 + findCol(code.slice(7, 10).toList))

    def missingSeat: Int = {
      val min = allIds.min
      val max = allIds.max

      ((min to max).toSet -- allIds).head
    }
